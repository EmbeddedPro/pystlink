...
class StlinkUsbConnector():
    ...
    def _write(self, data, tout=200):
        ...
        if count != len(data):
            raise pystlink.stlinkex.StlinkException("Error, only %d Bytes was transmitted to ST-Link instead of expected %d" % (count, len(data)))
        ...
        while (True):
            try:
                if len(cmd) > self.STLINK_CMD_SIZE_V2:
                    raise pystlink.stlinkex.StlinkException("Error too many Bytes in command: %d, maximum is %d" % (len(cmd), self.STLINK_CMD_SIZE_V2))
                # pad to 16 bytes
                ...
                if retry:
                    retry -= 1
                    continue
                raise pystlink.stlinkex.StlinkException("USB Error: %s" % e)
            return None
