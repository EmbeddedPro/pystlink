...
import argparse
import time
import pystlink.stlinkusb
import pystlink.stlinkv2
import pystlink.stm32
import pystlink.stm32fp
import pystlink.stm32fs
import pystlink.stm32devices
import pystlink.stlinkex
import pystlink.dbg
import pystlink.srec
...
class PyStlink():
    ...
    def find_mcus_by_core(self):
        ...
        if cpuid == 0:
            raise pystlink.stlinkex.StlinkException('Not connected to CPU')
        self._dbg.verbose("CPUID:  %08x" % cpuid)
        partno = 0xfff & (cpuid >> 4)
        for mcu_core in pystlink.stm32devices.DEVICES:
            if mcu_core['part_no'] == partno:
                self._mcus_by_core = mcu_core
                return
        raise pystlink.stlinkex.StlinkException('PART_NO: 0x%03x is not supported' % partno)
        ...
        for idcode_reg in idcode_regs:
            ...
            for mcu_devid in self._mcus_by_core['devices']:
                if mcu_devid['dev_id'] == devid:
                    ...
                    return
        raise pystlink.stlinkex.StlinkException('DEV_ID: 0x%03x is not supported' % devid)
    ...
    def find_mcus_by_flash_size(self):
        ...
        if not self._mcus:
            raise pystlink.stlinkex.StlinkException('Connected CPU with DEV_ID: 0x%03x and FLASH size: %dKB is not supported' % (
    ...
    def fix_cpu_type(self, cpu_type):
        ...
        if cpu_type.startswith('STM32'):
            ...
            return cpu_type
        raise pystlink.stlinkex.StlinkException('"%s" is not STM32 family' % cpu_type)
        ...
        for detected_cpu in self._mcus:
            for expected_cpu in expected_cpus:
                ...
                if detected_cpu['type'].startswith(expected_cpu):
                    ...
                    break
        if not cpus:
            raise pystlink.stlinkex.StlinkException('Connected CPU is not %s but detected is %s %s' % (
                ','.join(expected_cpus),
    ...
    def load_driver(self):
        ...
        if flash_driver == 'STM32FP':
            self._driver = pystlink.stm32fp.Stm32FP(self._stlink, dbg=self._dbg)
        elif flash_driver == 'STM32FPXL':
            self._driver = pystlink.stm32fp.Stm32FPXL(self._stlink, dbg=self._dbg)
        elif flash_driver == 'STM32FS':
            self._driver = pystlink.stm32fs.Stm32FS(self._stlink, dbg=self._dbg)
        else:
            self._driver = pystlink.stm32.Stm32(self._stlink, dbg=self._dbg)

    def detect_cpu(self, expected_cpus, unmount=False):
        self._connector = pystlink.stlinkusb.StlinkUsbConnector(dbg=self._dbg)
        if unmount:
            self._connector.unmount_discovery()
        self._stlink = pystlink.stlinkv2.Stlink(self._connector, dbg=self._dbg)
    ...
    def read_file(self, filename):
        ...
        with open(filename, 'rb') as f:
            ...
            return [(None, data)]
        raise pystlink.stlinkex.StlinkException("Error reading file")
    ...
    def cmd_read(self, params):
        ...
        if cmd == 'flash':
            ...
            raise pystlink.stlinkex.StlinkExceptionBadParam()
        data = self._driver.get_mem(addr, size)
    ...
    def cmd_set(self, params):
        ...
        params = params[1:]
        if not params:
            raise pystlink.stlinkex.StlinkExceptionBadParam('Missing argument')
        data = int(params[0], 0)
    ...
    def cmd_write(self, params):
        ...
        if len(mem) == 1 and mem[0][0] is None:
            data = mem[0][1]
            if len(params) != 1:
                raise pystlink.stlinkex.StlinkExceptionBadParam('Address is not set')
            if params[0] == 'sram':
                addr = self._driver.SRAM_START
                if len(data) > self._sram_size * 1024:
                    raise pystlink.stlinkex.StlinkExceptionBadParam('Data are bigger than SRAM')
            else:
                addr = int(params[0], 0)
            self._driver.set_mem(addr, data)
            return
        if params:
            raise pystlink.stlinkex.StlinkException('Address for write is set by file')
    ...
    def cmd_flash(self, params):
        ...
        if params and params[0] == 'verify':
            verify = True
            params = params[1:]
        start_addr = pystlink.stm32.Stm32.FLASH_START
        if len(mem) == 1 and mem[0][0] is None:
            if params:
                start_addr = int(params[0], 0)
                params = params[1:]
        if params:
            raise pystlink.stlinkex.StlinkExceptionBadParam('Address for write is set by file')
        for addr, data in mem:
            if addr is None:
                addr = start_addr
    ...
    def start(self):
        ...
        self._dbg = pystlink.dbg.Dbg(args.verbosity)
        runtime_status = 0
        try:
            self.detect_cpu(args.cpu, not args.no_unmount)
            if args.action and self._driver is None:
                raise pystlink.stlinkex.StlinkExceptionCpuNotSelected()
            for action in args.action:
                self._dbg.verbose('CMD: %s' % action)
                try:
                    self.cmd(action.split(':'))
                except pystlink.stlinkex.StlinkExceptionBadParam as e:
                    raise e.set_cmd(action)
        except (pystlink.stlinkex.StlinkExceptionBadParam, pystlink.stlinkex.StlinkException) as e:
            self._dbg.error(e)
            runtime_status = 1
        ...
        if self._stlink:
            ...
            except pystlink.stlinkex.StlinkException as e:
                self._dbg.error(e)
                runtime_status = 1
